...
/* -------------admin_password_change------------*/
	...
	public function add_new_task($data){
		...
		$mobile_number = $this->test_form_input_data($data['mobile_number']);
		$bike_model = $this->test_form_input_data($data['bike_model']);
		$assign_to = $this->test_form_input_data($data['assign_to']);
	
		try {
			$add_task = $this->db->prepare("INSERT INTO task_info (t_title, t_description, t_start_time, t_end_time, mobile_number, bike_model, t_user_id) VALUES (:x, :y, :z, :a, :b, :c, :d)");
			...
			$add_task->bindParam(':b', $mobile_number);
			$add_task->bindParam(':c', $bike_model);
			$add_task->bindParam(':d', $assign_to);
	...
	public function update_task_info($data, $task_id, $user_role){
		...
		try {
			$update_task = $this->db->prepare("UPDATE task_info SET t_title = :x, t_description = :y, t_start_time = :z, t_end_time = :a, mobile_number = :b, bike_model = :c, t_user_id = :d, status = :e WHERE task_id = :id ");
			...
			$update_task->bindParam(':b', $mobile_number);
			$update_task->bindParam(':c', $bike_model);
